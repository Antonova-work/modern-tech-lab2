...
/*
В файле указан METHOD, потому что аннотация предназначена для описания тестовых сценариев,
которые в коде представлены именно методами (testAdd, testMultiply). Если заменить METHOD на TYPE,
аннотацию можно будет ставить только над классами или интерфейсами, и при попытке оставить её над
методами возникнет ошибка компиляции. Если заменить на FIELD, аннотацию разрешат использовать только
для переменных (полей) класса. В обоих случаях текущий код перестанет работать, так как механизм рефлексии
в методе main ищет эти метаданные именно у методов, а компилятор не позволит пометить ими функции.
Таким образом, METHOD строго ограничивает область применения аннотации там, где она логически необходима.
 */
